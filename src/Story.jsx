...
const appSetting = {databaseURL: "https://sysufe-62ef7-default-rtdb.firebaseio.com/",}
...
export default function Story() {
  ...
  useEffect(() => {
    if (selectedValue) {
      onValue(ref(database, `List/${selectedValue.toUpperCase()}`), getFromDb)  
    }
    setReveal({})  
  }, [selectedValue])  
  ...
  useEffect(() => {
    if (check) {
      onValue(ref(database, `List/${check.toUpperCase()}`), getFromDb)  
    }
  ...
  useEffect(() => {
    if (search) {
      onValue(ref(database, `List/${search.toUpperCase()}`), getFromDb)  
    }
    setReveal({})  
  }, [search])  
  ...
  function handleSubmit(e) {
    ...
    if (subject && describe && selectedCategory) {
      push(ref(database, `List/${selectedCategory}`), Data).then(() => {
        setOpen(true)  
      })  
      ...
      if (selectedCategory) {
        if (
          ...
          push(cat, selectedCategory).then(() => {
            setOpen(true)  
          })  
        }
        clear()  
  ...
  function getFromDb(snapshot) {
    if (snapshot.exists()) {
      ...
      setMappable(
        Object.entries(
          Object.entries(snapshot.val()).length ? snapshot.val() : []
        )
      )  
    } else {
      setStories(0)  
      setMappable(Object.entries([]))  
    }
  }
      ...
      <div className="story-section">
        ...
        <section className="section-2">
          <div className="section-2-head">
            ...
            {/* Change - "Read their stories" to Read stories on <Category Name> */}
            <h1>Read Stories On {category}</h1>
          ...
          <div className="filter">
            {stories === 0 ? (
              ...
              <h1 className="total-story">
                <span>
                  {stories === 1 ? `${stories} story` : `${stories} stories`}
                </span>{" "}
                for you to read
              </h1>
            )}
